...
public class A {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (dfs.treeHasCycle) {
            writer.write(Integer.toString(-1).getBytes());
            writer.close();
            return;
        }

        Collections.reverse(dfs.vertexesInTopSort);
        for (int vertex : dfs.vertexesInTopSort) {
            writer.write(Integer.toString(vertex + 1).getBytes());
