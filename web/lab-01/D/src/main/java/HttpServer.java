...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        String decodedUri = java.net.URLDecoder.decode(request.getUri());
        File file = new File(root, decodedUri);
        if (file.isFile()) {
            byte[] body = readFile(file);

            //System.out.println(Arrays.toString(body).hashCode() + " ||| " + request.headers.get("If-None-Match"));
            if (!Integer.toString(Arrays.toString(body).hashCode()).equals(request.headers.get("If-None-Match"))) {
            ...
            response.setHeader("ETag", Integer.toString(Arrays.toString(body).hashCode()));
        } else if (file.isDirectory()) {
            File defaultFile;
            if (decodedUri.charAt(decodedUri.length() - 1) == '/') {
                defaultFile = new File(root, decodedUri + "index.html");
            } else {
                defaultFile = new File(root,decodedUri + "/index.html");
            }

            byte[] body = readFile(defaultFile);
            if (!Integer.toString(Arrays.toString(body).hashCode()).equals(request.headers.get("If-None-Match"))) {
                response.setStatusCode(200);
                response.setBody(body);
                response.setHeader("Content-Length", Integer.toString(body.length));
                response.setHeader("Content-Type", getContentType(defaultFile));
            } else {
                response.setStatusCode(304);
            }
            response.setHeader("ETag", Integer.toString(Arrays.toString(body).hashCode()));
    ...
    public static void main(String[] args) throws IOException {
        new HttpServer(new File("/Users/admin/Desktop/Web/server14-v2/static"), 8088);
    }
}
